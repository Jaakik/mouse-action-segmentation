...
def bounding_box(data):
    ...
    for sequence_key in tqdm(sequence_train_names):
        ...
        x_left = np.maximum(x_min_1, x_min_2)
        y_top = np.maximum(y_min_1, y_min_2)
        x_right = np.minimum(x_max_1, x_max_2)
        y_bottom = np.minimum(y_max_1, y_max_2)
...
def center_of_mass(data):
    ...
    for sequence_key in tqdm(sequence_train_names):
        ...
        data[sequence_key] = np.zeros((shape[0], shape[1]+7))
        # keypoints = single_sequence["keypoints"]
        ...
        data[sequence_key][:, 0:shape[1]] = single_sequence[:, 0:shape[1]]
        data[sequence_key][:, shape[1]:shape[1]+2] = mouse1_center
        data[sequence_key][:, shape[1]+2:shape[1]+4] = mouse2_center
        data[sequence_key][:, shape[1]+4] = mouse1_dispersion
        data[sequence_key][:, shape[1]+5] = mouse2_dispersion
        data[sequence_key][:, shape[1]+6] = distance_between_centers
    return data
    ...
    for seq_id in tqdm(list(data_dict.keys())):
        ...
        for frame in range(n_frames):
            ...
            if (frame < n_frames-1):
                ...
                joints_velocity_1 = np.diagonal(cdist(mouse1_xy_fl, mouse1_xy, 'euclidean'))*fps*int_factor
                joints_velocity_2 = np.diagonal(cdist(mouse2_xy_fl, mouse2_xy, 'euclidean'))*fps*int_factor
        ...
        data_dict[seq_id][:, 0:35] = keypoints
        data_dict[seq_id][:, 35:] = feature_f
        del feature_f
    return data_dict
