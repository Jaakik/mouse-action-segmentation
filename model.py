    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, device):
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                batch_input_1, batch_target_1, mask_1, batch_input_2, batch_target_2, mask_2,batch_input_3, batch_target_3, mask_3, behaviour,annotator = batch_gen.next_batch(batch_size)
                batch_input_1, batch_target_1, mask_1 = batch_input_1.to(device), batch_target_1.to(device), mask_1.to(device)
                batch_input_2, batch_target_2, mask_2 = batch_input_2.to(device), batch_target_2.to(device), mask_2.to(device)
                batch_input_3, batch_target_3, mask_3 = batch_input_3.to(device), batch_target_3.to(device), mask_3.to(device)

                optimizer.zero_grad()
                #multitask learning
                predictions_1 ,predictions_2,predictions_3  = self.model(batch_input_1,batch_input_2, batch_input_3,behaviour, annotator)
                ...
                for p in predictions_1:
                    loss += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes_1), batch_target_1.view(-1))
                ...
                for p in predictions_3:
                    loss += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes_3), batch_target_3.view(-1))
                    loss += 0.15 * torch.mean(torch.clamp(
                        self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)), min=0,
                        max=16) * mask_3[:, :, 1:])
            ...
            logger.info(
                "[epoch %d]: epoch loss = %f,   acc_1 = %f,acc_3 = %f, acc_3 = %f " % (epoch + 1, epoch_loss / len(batch_gen.list_of_examples_1),
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict, device, sample_rate):
        ...
        with torch.no_grad():
            ...
            for vid in list_of_vids:
                ...
                predictions_1, predictions_3 = self.model(input_x,input_x)
                _, predicted_1 = torch.max(predictions_1[-1].data, 1)
                _, predicted_3 = torch.max(predictions_3[-1].data, 1)

                predicted_1 = predicted_1.squeeze()
                predicted_3 = predicted_3.squeeze()

                recognition = []
                for i in range(len(predicted_1)):
                    recognition = np.concatenate((recognition, [list(actions_dict.keys())[list(actions_dict.values()).index(predicted_1[i].item())]]*sample_rate))
